...
const projects = [
  { id:'today-day-one', no:'01', title:'오늘부터 1일', category:'GAME', kind:'STORY SIMULATION GAME', platform:'PC / WEB', year:'2026', tone:'sunset', tagline:'30 DAYS. ONE LOST MEMORY. YOUR CHOICE.', description:'기억을 잃은 주인공이 30일 동안 자신의 과거와 관계를 되찾는 스토리 기반 연애 시뮬레이션.' },
  { id:'health-guardians', no:'02', title:'경북 건강수호대', category:'VR / XR', kind:'VR EXPERIENCE GAME', platform:'META QUEST · EYE TRACKING', year:'2026', tone:'vr', tagline:'LOOK. BLINK. DESTROY.', description:'시선과 눈 깜빡임을 활용해 혈관 속 세균을 제거하는 체험형 VR 게임.' },
  { id:'chicken-air', no:'03', title:'CHICKEN AIR', category:'GAME', kind:'CASUAL SHOOTING GAME', platform:'WEB / MOBILE', year:'2026', tone:'air', tagline:'FLY HIGH. FIRE FAST.', description:'닭을 타고 하늘을 날며 알을 발사해 적을 처치하는 캐주얼 슈팅 게임.' },
];
...
export default function Home() {
  ...
  return <main>
    ...
    {selected&&<div className="modal-backdrop" role="presentation" onMouseDown={()=>setSelected(null)}><section className={`project-modal ${selected.tone}`} role="dialog" aria-modal="true" aria-labelledby="project-title" onMouseDown={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setSelected(null)} aria-label="프로젝트 닫기">×</button><span className="eyebrow">{selected.no} / {selected.category}</span><h2 id="project-title">{selected.title}</h2><h3>{selected.tagline}</h3><p>{selected.description}</p><dl><div><dt>TYPE</dt><dd>{selected.kind}</dd></div><div><dt>PLATFORM</dt><dd>{selected.platform}</dd></div><div><dt>YEAR</dt><dd>{selected.year}</dd></div></dl><a className="button primary" href="#contact" onClick={()=>setSelected(null)}>START A SIMILAR PROJECT ↗</a></section></div>}
  </main>;
}
