...
export default function Home() {
  ...
  return <main>
    ...
    {selected&&<div className="modal-backdrop" role="presentation" onMouseDown={()=>setSelected(null)}><section className={`project-modal ${selected.tone} ${selected.video?'with-video':''}`} role="dialog" aria-modal="true" aria-labelledby="project-title" aria-describedby="project-description" onMouseDown={e=>e.stopPropagation()}><div className="modal-rail"><span>MDS / PROJECT ARCHIVE</span><span className="online-dot">ONLINE</span></div><button className="modal-close" onClick={()=>setSelected(null)} aria-label="프로젝트 닫기">×</button><div className="modal-info"><span className="eyebrow">{selected.no} / {selected.category}</span><h2 id="project-title">{selected.title}</h2><h3>{selected.tagline}</h3><p id="project-description">{selected.description}</p><dl><div><dt>TYPE</dt><dd>{selected.kind}</dd></div><div><dt>PLATFORM</dt><dd>{selected.platform}</dd></div><div><dt>YEAR</dt><dd>{selected.year}</dd></div></dl><div className="modal-actions">{selected.url&&<a className="button primary" href={selected.url} target="_blank" rel="noreferrer">PLAY NOW ↗</a>}<a className="button" href="#contact" onClick={()=>setSelected(null)}>START A SIMILAR PROJECT</a></div></div>{selected.video&&<div className="project-video"><div className="video-heading"><p className="eyebrow">GAME TRAILER / 소개 영상</p><span>PROJECT FILM</span></div><div className="video-frame"><iframe src={selected.video} title={`${selected.title} 게임 소개 영상`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen loading="lazy"/></div><a href="https://youtu.be/N7Bc-XWqu3M" target="_blank" rel="noreferrer">YOUTUBE에서 전체 화면으로 보기 ↗</a></div>}</section></div>}
  </main>;
}
